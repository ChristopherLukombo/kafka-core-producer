...
public class CommodityService {
	...
	private static final double MAX_AJUSTEMENT = 1.05d;
	private static final double MIN_AJUSTEMENT = 0.95d;
	
	static {
		var timestamp = System.currentTimeMillis();
		
		COMMODITY_BASE.put(COPPER, new Commodity(COPPER, 5_900.57, "tonne", timestamp));
		COMMODITY_BASE.put(GOLD, new Commodity(GOLD, 5_900.57, "ounce", timestamp));
	}
	...
	public Commodity createDummyCommodity(String name) {
		...
		var newPrice = basePrice * ThreadLocalRandom.current().nextDouble(MIN_AJUSTEMENT, MAX_AJUSTEMENT);
